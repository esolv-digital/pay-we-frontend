...
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/countries${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/countries] GET Error:', apiError.response?.data?.message || 'Unknown error');
...
export async function POST(request: Request) {
  try {
    ...
    const laravelClient = createLaravelClient(token);
    const response = await laravelClient.post<{ data: unknown }>('/admin/countries', body);
    ...
    console.error('[/api/admin/countries] POST Error:', apiError.response?.data?.message || 'Unknown error');
