...
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/reports/revenue/export${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/reports/revenue/export] GET Error:', apiError.response?.data?.message || 'Unknown error');
