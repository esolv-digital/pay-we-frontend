...
/**
 * GET /api/admin/login-attempts/statistics
...
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/login-attempts/statistics${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/login-attempts/statistics] GET Error:', apiError.response?.data?.message || 'Unknown error');
