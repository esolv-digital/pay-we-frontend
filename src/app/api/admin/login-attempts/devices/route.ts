...
/**
 * GET /api/admin/login-attempts/devices
 * List user devices
 */
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/login-attempts/devices${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/login-attempts/devices] GET Error:', apiError.response?.data?.message || 'Unknown error');
