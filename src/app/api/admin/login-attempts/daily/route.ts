...
/**
 * GET /api/admin/login-attempts/daily
...
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/login-attempts/daily${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/login-attempts/daily] GET Error:', apiError.response?.data?.message || 'Unknown error');
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch daily login data',
