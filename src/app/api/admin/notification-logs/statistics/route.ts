...
/**
 * GET /api/admin/notification-logs/statistics
...
export async function GET(request: Request) {
  try {
    ...
    const response = await laravelClient.get<{ data: unknown }>(
      `/admin/notification-logs/statistics${queryString ? `?${queryString}` : ''}`
    );
    ...
    console.error('[/api/admin/notification-logs/statistics] GET Error:', apiError.response?.data?.message || 'Unknown error');
