...
/**
 * GET /api/vendors/[vendor_slug]/transactions/export/summary
 * Get export summary before performing actual export
 * Returns total records, file size estimate, metrics, and whether export is possible
 */
export async function GET(
  ...
  try {
    ...
    const apiUrl = `/vendors/${vendor_slug}/transactions/export/summary${queryString ? `?${queryString}` : ''}`;
    ...
    return NextResponse.json(
      {
        success: false,
        status: 'error',
        message: apiError.response?.data?.message || 'Failed to fetch export summary',
