...
/**
 * GET /api/vendors/[vendor_slug]/transactions/metrics
 * Get aggregated transaction metrics for a vendor
 * Supports all filtering parameters to calculate metrics for specific subsets
 */
export async function GET(
  ...
  try {
    ...
    const apiUrl = `/vendors/${vendor_slug}/transactions/metrics${queryString ? `?${queryString}` : ''}`;
    ...
    return NextResponse.json(
      {
        success: false,
        status: 'error',
        message: apiError.response?.data?.message || 'Failed to fetch transaction metrics',
